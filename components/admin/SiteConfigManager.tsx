...
import { Settings, Save, Globe, Users, Loader2, Image as ImageIcon, Calendar, Plus, Trash2, Edit, Eye, Smartphone, Monitor, Info, Megaphone, AlertTriangle, CheckCircle, CreditCard, ToggleLeft, ToggleRight, Layout, AlertOctagon } from 'lucide-react';
            ...
            <div className="p-6 flex-grow overflow-y-auto">
                ...
                {activeTab === 'system' && (
                    <div className="space-y-8 animate-in fade-in">
                        ...
                        {/* DATOS BANCARIOS */}
                        <div className="bg-white p-6 rounded-xl border border-gray-200">
                            <h4 className="font-bold text-gray-800 mb-4 flex items-center gap-2"><CreditCard className="w-5 h-5 text-green-600"/> Datos Facturación Global</h4>
                            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                                <div>
                                    <label className="block text-xs font-bold text-gray-500 uppercase mb-1">Razón Social</label>
                                    <input type="text" className="w-full p-2 border rounded text-sm" value={config.billing?.companyName} onChange={e => setConfig({...config, billing: {...config.billing, companyName: e.target.value}})} />
                                </div>
                                <div>
                                    <label className="block text-xs font-bold text-gray-500 uppercase mb-1">IBAN Cobros</label>
                                    <input type="text" className="w-full p-2 border rounded text-sm font-mono" value={config.billing?.companyIban} onChange={e => setConfig({...config, billing: {...config.billing, companyIban: e.target.value}})} placeholder="ES..." />
                                </div>
                            </div>
                        </div>
                        ...
                        {/* MÓDULOS */}
                        <div className="bg-white p-6 rounded-xl border border-gray-200">
                            <h4 className="font-bold text-gray-800 mb-4 flex items-center gap-2"><Layout className="w-5 h-5 text-purple-600"/> Control de Módulos (Visibilidad)</h4>
                            <div className="space-y-4">
                                <label className="flex items-center justify-between p-3 bg-gray-50 rounded-lg cursor-pointer">
                                    <span className="text-sm font-medium">Mostrar sección BLOG</span>
                                    <div className="relative inline-flex items-center cursor-pointer">
                                        <input type="checkbox" className="sr-only peer" checked={config.modules?.showBlog} onChange={e => setConfig({...config, modules: {...config.modules, showBlog: e.target.checked}})} />
                                        <div className="w-11 h-6 bg-gray-200 peer-focus:outline-none rounded-full peer peer-checked:after:translate-x-full peer-checked:after:border-white after:content-[''] after:absolute after:top-[2px] after:left-[2px] after:bg-white after:border-gray-300 after:border after:rounded-full after:h-5 after:w-5 after:transition-all peer-checked:bg-green-600"></div>
                                    </div>
                                </label>
                                <label className="flex items-center justify-between p-3 bg-gray-50 rounded-lg cursor-pointer">
                                    <span className="text-sm font-medium">Mostrar sección DESCUENTOS</span>
                                    <div className="relative inline-flex items-center cursor-pointer">
                                        <input type="checkbox" className="sr-only peer" checked={config.modules?.showDiscounts} onChange={e => setConfig({...config, modules: {...config.modules, showDiscounts: e.target.checked}})} />
                                        <div className="w-11 h-6 bg-gray-200 peer-focus:outline-none rounded-full peer peer-checked:after:translate-x-full peer-checked:after:border-white after:content-[''] after:absolute after:top-[2px] after:left-[2px] after:bg-white after:border-gray-300 after:border after:rounded-full after:h-5 after:w-5 after:transition-all peer-checked:bg-green-600"></div>
                                    </div>
                                </label>
                            </div>
                        ...
                        <div className="bg-red-50 p-6 rounded-xl border border-red-200">
                            ...
                            <label className="flex items-center justify-between cursor-pointer">
                                <div>
                                    <span className="text-sm font-bold text-red-800">Modo Mantenimiento</span>
                                    <p className="text-xs text-red-600 mt-1">Si activas esto, la web pública mostrará una pantalla de "En construcción". Solo los administradores podrán acceder.</p>
                                </div>
                                <div className="relative inline-flex items-center cursor-pointer">
