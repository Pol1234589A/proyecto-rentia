        ...
        <div className="min-h-screen bg-slate-50 flex flex-col font-sans">
            ...
            <header className="bg-white/95 backdrop-blur-md border-b border-gray-200 sticky top-0 z-50 transition-all duration-300 shadow-sm">
                <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 h-16 flex justify-between items-center">
                    <div className="flex items-center gap-4">
                        ...
                        <span className="hidden sm:block text-slate-500 text-xs font-bold tracking-widest uppercase">{t('opportunities.hero.badge')}</span>
                    </div>
                    <div className="flex items-center gap-4">
                        <a
                            href={calendarUrl}
                        ...
                        >
                            <Calendar className="w-3 h-3" /> {t('opportunities.landing.schedule' as any) || 'Agendar Reunión'}
                        </a>
                        <a
                        ...
                        >
                            <Globe className="w-3 h-3" /> <span className="hidden sm:inline">{t('header.home')}</span>
                        </a>
                    </div>
                </div>
            </header>
            ...
            <section className="bg-slate-900 text-white pt-20 pb-24 md:pt-32 md:pb-40 relative overflow-hidden">
                ...
                <div className="container mx-auto px-4 relative z-10 max-w-4xl text-center">
                    <div className="inline-flex items-center gap-2 bg-white/10 backdrop-blur border border-white/20 px-4 py-1.5 rounded-full mb-8 shadow-lg shadow-white/5 animate-in fade-in slide-in-from-bottom-4 duration-700">
                        ...
                        <span className="text-xs font-bold tracking-widest uppercase text-gray-200">{t('opportunities.hero.access')}</span>
                    </div>
            ...
            <div className="bg-white border-b border-gray-200 py-4 relative z-20 -mt-8 mx-4 sm:mx-auto max-w-5xl rounded-xl shadow-lg flex justify-around items-center text-center animate-in fade-in slide-in-from-bottom-8 duration-700 delay-500">
                <div className="px-4">
                    ...
                    <p className="text-[10px] text-gray-500 uppercase tracking-wide font-bold">{t('opportunities.stats.yield')}</p>
                </div>
                <div className="w-px h-8 bg-gray-200"></div>
                <div className="px-4">
                    <p className="text-2xl font-bold text-slate-800 font-display">100%</p>
                    <p className="text-[10px] text-gray-500 uppercase tracking-wide font-bold">{t('opportunities.stats.management')}</p>
                </div>
                <div className="w-px h-8 bg-gray-200"></div>
                <div className="px-4">
                    <p className="text-2xl font-bold text-slate-800 font-display">Murcia</p>
                    <p className="text-[10px] text-gray-500 uppercase tracking-wide font-bold">{t('opportunities.stats.location')}</p>
                </div>
            </div>
            ...
            <main id="opp-grid" className="flex-grow container mx-auto px-4 py-20 max-w-7xl">
                <div className="flex flex-col md:flex-row md:items-end justify-between mb-8 pb-4 border-b border-gray-200 gap-4">
                    <div>
                        <h2 className="text-2xl font-bold text-slate-900 font-display">{t('opportunities.detail.financial_study')}</h2>
                        <p className="text-sm text-gray-500 mt-1">{t('opportunities.filters.updated')}</p>
                    </div>
                    ...
                    <div className="flex items-center gap-3">
                        ...
                        <span className="text-xs font-bold bg-slate-100 text-slate-600 px-3 py-2 rounded-lg">
                            {filteredOpps.length} {t('opportunities.filters.results')}
                        </span>
                    </div>
                </div>
                ...
                {showFilters && (
                    <div className="bg-white p-6 rounded-xl border border-gray-200 shadow-sm mb-8 animate-in slide-in-from-top-2">
                        <div className="flex justify-between items-center mb-4">
                            <h3 className="text-sm font-bold text-slate-800 uppercase tracking-wide">{t('opportunities.filters.subtitle')}</h3>
                            <button onClick={() => { setFilterYield(0); setMaxPrice(0); }} className="text-xs text-rentia-blue font-bold hover:underline">{t('opportunities.filters.clear')}</button>
                        </div>
                        <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-4 gap-6">
                            <div>
                                <label className="block text-xs font-medium text-gray-500 mb-2">{t('opportunities.filters.min_yield')}</label>
                                <div className="flex gap-2">
                                    {[0, 7, 8, 9, 10].map(val => (
                                        ...
                                        >
                                            {val === 0 ? t('rooms.filter.all_zones') : `+${val}%`}
                                        </button>
                                    ))}
                                </div>
                            </div>
                            <div>
                                <label className="block text-xs font-medium text-gray-500 mb-2">{t('opportunities.filters.max_price')}</label>
                                <select
                                    value={maxPrice}
                                ...
                                >
                                    <option value="0">{t('opportunities.filters.any_price')}</option>
                                    <option value="100000">{t('opportunities.filters.until')} 100.000 €</option>
                                    <option value="150000">{t('opportunities.filters.until')} 150.000 €</option>
                                    <option value="200000">{t('opportunities.filters.until')} 200.000 €</option>
                                    <option value="300000">{t('opportunities.filters.until')} 300.000 €</option>
                                </select>
                            </div>
                        </div>
                    </div>
                ...
                {filteredOpps.length > 0 ? (
                    ...
                    <div className="bg-white p-16 rounded-2xl shadow-sm text-center border border-gray-200 max-w-2xl mx-auto">
                        ...
                        <h3 className="text-xl font-bold text-slate-900 mb-2">{t('opportunities.detail.no_results')}</h3>
                        <p className="text-gray-500 leading-relaxed mb-8">
                            {t('opportunities.detail.try_adjust')}
                        </p>
                        <button
                        ...
                        >
                            {t('opportunities.detail.see_all')}
                        </button>
                    </div>
                )}
            </main>
            ...
            <footer className="bg-white border-t border-gray-200 pt-12 pb-8">
                <div className="container mx-auto px-4">
                    <div className="grid grid-cols-1 md:grid-cols-3 gap-8 mb-12">
                        <div>
                            ...
                            <p className="text-sm text-gray-500 leading-relaxed">
                                {t('opportunities.detail.footer_desc')}
                            </p>
                        </div>
                        <div>
                            <h4 className="font-bold text-slate-900 mb-4 text-sm uppercase tracking-wide">{t('opportunities.detail.contact_title')}</h4>
                            <ul className="space-y-3 text-sm text-gray-600">
                                ...
                                <li>
                                    <a href={`tel:+${config.directorContact.phone}`} className="flex items-center gap-2 hover:text-rentia-blue transition-colors">
                                        <Phone className="w-4 h-4" /> +{config.directorContact.phone.slice(0, 2)} {config.directorContact.phone.slice(2, 5)} {config.directorContact.phone.slice(5, 7)} {config.directorContact.phone.slice(7, 9)} {config.directorContact.phone.slice(9)}
                                    </a>
                                </li>
                            </ul>
                        </div>
                        <div>
                            <h4 className="font-bold text-slate-900 mb-4 text-sm uppercase tracking-wide">{t('opportunities.detail.guarantees')}</h4>
                            <ul className="space-y-3 text-sm text-gray-600">
                                <li className="flex items-center gap-2"><Check className="w-4 h-4 text-green-500" /> {t('opportunities.detail.g1')}</li>
                                <li className="flex items-center gap-2"><Check className="w-4 h-4 text-green-500" /> {t('opportunities.detail.g2')}</li>
                                <li className="flex items-center gap-2"><Check className="w-4 h-4 text-green-500" /> {t('opportunities.detail.g3')}</li>
                            </ul>
                        </div>
                    </div>
                    ...
                    <div className="border-t border-gray-100 pt-8 flex flex-col md:flex-row justify-between items-center text-xs text-gray-400">
                        ...
                        <p className="mt-2 md:mt-0">{t('opportunities.detail.confidential_long')}</p>
                    </div>
                </div>
            </footer>
        </div>
