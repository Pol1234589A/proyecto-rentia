...
export const WhatsAppButton: React.FC = () => {
  ...
  return (
    ...
    <div className="hidden md:flex fixed bottom-6 right-6 z-50 print:hidden flex-col items-end pointer-events-none">
      ...
      {/* Menu Popup */}
      <div
        className={`mb-4 flex flex-col gap-3 transition-all duration-300 origin-bottom-right ${isOpen ? 'opacity-100 scale-100 translate-y-0 pointer-events-auto' : 'opacity-0 scale-90 translate-y-10 pointer-events-none'
          }`}
      >
        {/* Director Button */}
        <a
          ...
          className={`flex items-center gap-3 p-4 rounded-xl shadow-xl border-2 transition-all w-72 group relative overflow-hidden touch-manipulation cursor-pointer ${directorStatus.isOpen
              ? 'bg-white border-green-500 hover:bg-green-50'
              : 'bg-white border-gray-200 hover:bg-gray-50 grayscale-[0.5] hover:grayscale-0'
            }`}
        >
          <div className={`w-12 h-12 rounded-full flex items-center justify-center flex-shrink-0 transition-colors overflow-hidden ${directorStatus.isOpen ? 'bg-blue-100 text-rentia-blue' : 'bg-gray-100 text-gray-500'
            }`}>
      ...
      <button
        ...
        className={`flex items-center justify-center w-16 h-16 rounded-full shadow-2xl transition-all duration-300 focus:outline-none focus:ring-4 focus:ring-green-200 pointer-events-auto touch-manipulation cursor-pointer ${isOpen ? 'bg-gray-800 rotate-90' : 'bg-[#25D366] hover:bg-[#20BA5C] hover:scale-105'
          }`}
