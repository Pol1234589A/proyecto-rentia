...
export const OpportunityCard: React.FC<Props> = ({ opportunity, onClick }) => {
  ...
  const monthlyIncome = (opportunity.financials?.monthlyRentProjected || 0) > 0 
      ? (opportunity.financials?.monthlyRentProjected || 0) 
      : (opportunity.financials?.monthlyRentTraditional || 0);

  const purchasePrice = opportunity.financials?.purchasePrice || 0;
  ...
  const finalTotalInvestment = (opportunity.financials?.totalInvestment || purchasePrice) + agencyFeeTotal;
      
  const grossYield = finalTotalInvestment > 0 ? ((monthlyIncome * 12) / finalTotalInvestment) * 100 : 0;

  let displayAddress = opportunity.address || 'Ubicación';
  ...
  return (
    ...
    >
      <div className="relative h-64 overflow-hidden bg-gray-100">
        ...
        <img 
          src={opportunity.images?.[0] || 'https://placehold.co/600x400?text=No+Image'} 
      ...
      <div className="p-5 flex flex-col justify-between flex-grow bg-white">
        <div>
            ...
            <div className="grid grid-cols-2 gap-3 mb-5">
                <div className="p-3 rounded-lg bg-gray-50 border border-gray-100">
                    ...
                    <p className="font-bold text-gray-900 text-lg">{purchasePrice.toLocaleString('es-ES')} €</p>
                </div>
            ...
            <div className="flex justify-between items-center text-xs text-gray-500 font-medium mb-4 px-1">
                <div className="flex items-center gap-1.5">
                    ...
                    <span>{opportunity.specs?.rooms || 0} {t('opportunities.card.rooms')}</span>
                </div>
                <div className="flex items-center gap-1.5">
                    <Maximize className="w-4 h-4 text-gray-300" />
                    <span>{opportunity.specs?.sqm || 0} m²</span>
                </div>
            </div>
        </div>
        ...
        <div className="mt-auto grid grid-cols-2 gap-3">
            ...
            <button 
                onClick={(e) => { e.stopPropagation(); onClick(opportunity.id); }}
                className="bg-rentia-black text-white font-bold py-3 px-2 rounded-xl flex items-center justify-center gap-2 hover:bg-gray-800 transition-all shadow-md text-xs"
            >
