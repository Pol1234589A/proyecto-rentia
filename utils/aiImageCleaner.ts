...
export const cleanImageWithAI = async (file: Blob | File, apiKey: string | undefined): Promise<Blob> => {
    ...
    try {
        ...
        console.warn("IA no detectó formato JSON válido, se devuelve imagen original.", e);
        return file;
    }
    ...
    if (analysis.hasWatermark && analysis.boxes && analysis.boxes.length > 0) {
        return new Promise((resolve) => {
            const img = new Image();
            img.src = base64Full;
            img.crossOrigin = "Anonymous"; // Importante para imágenes externas si el servidor lo permite
            img.onload = () => {
                ...
                analysis.boxes.forEach((box: number[]) => {
                    ...
                    // Estrategia de clonado: Copiar píxeles adyacentes
                    // Si el logo está arriba, cogemos de abajo. Si está abajo, de arriba.
                    const isTop = ymin < 0.2;
                    const sourceH = Math.min(50, img.height * 0.1); 

                    try {
                        if (isTop) {
                            // Copiar tira de abajo del logo y pegarla encima repetidamente si es necesario
                            // Simplificación: Copiar región adyacente inmediata
                            ctx.drawImage(
                                canvas, 
                                patchX, patchY + patchH + 2, patchW, sourceH, // Source (debajo)
                                patchX, patchY, patchW, patchH // Destino (sobre el logo)
                            );
                        } else {
                            // Copiar de arriba
                            ctx.drawImage(
                                canvas, 
                                patchX, patchY - sourceH - 2, patchW, sourceH, // Source (arriba)
                                patchX, patchY, patchW, patchH // Destino
                            );
                        }
                    } catch (e) {
            ...
            img.onerror = () => {
                console.warn("Error cargando imagen para procesar (posible CORS).");
                resolve(file);
            };
        });
    }
